...
const TopNav = () => {
  ...
  try {
    const response = await axios.get(
      `https://bussinesstoolcrm.up.railway.app/databank/search_in_databank/?q=${encodeURIComponent(searchTerm)}`,
      { headers: { Authorization: `Bearer ${accessToken}` } }
    );
  ...
  const newTimeout = setTimeout(async () => {
    ...
    try {
      const res = await axios.get(
        `https://bussinesstoolcrm.up.railway.app/databank/auto_complete_search_admin/?q=${encodeURIComponent(value)}`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
